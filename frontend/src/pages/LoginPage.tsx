...
// ─── Shared layout shell ─────────────────────────────────────────────────────
// Mobile: card flows from top, fully scrollable, no centering
// Desktop: centered with auto margins
function LoginShell({ children }: { children: React.ReactNode }) {
  return (
    <div
      className="min-h-dvh overflow-x-hidden bg-gradient-to-br from-violet-600 via-purple-600 to-fuchsia-500"
    ...
    >
      ...
      {/* Card container — top-aligned on mobile, centered on desktop */}
      <div className="relative z-10 px-4 pt-8 pb-6 md:pt-0 md:min-h-dvh md:flex md:items-center md:justify-center md:py-10">
        <div className="w-full max-w-md mx-auto bg-white/95 dark:bg-gray-900/95 backdrop-blur-xl rounded-3xl shadow-2xl shadow-violet-900/20 p-6 md:p-10">
          {children}
        </div>
      </div>
    </div>
...
export default function LoginPage() {
  ...
  return (
    <LoginShell>
      ...
      <div className={`flex flex-col items-center ${mode === 'register' ? 'mb-4' : 'mb-6'}`}>
        <div className="relative mb-2">
          <LogoMark size={mode === 'register' ? 44 : 56} className="rounded-2xl shadow-lg shadow-violet-500/20" />
        ...
        <h1 className={`font-bold text-gray-900 dark:text-gray-100 tracking-tight ${mode === 'register' ? 'text-lg' : 'text-xl'}`}>FinArch</h1>
        <p className="text-xs text-gray-400 dark:text-gray-500 mt-0.5 tracking-wide">{t('login.subtitle')}</p>
      </div>

      {justVerified && (
        <div className="mb-4 bg-emerald-50 dark:bg-emerald-900/30 border border-emerald-200 dark:border-emerald-700 text-emerald-700 dark:text-emerald-400 rounded-xl px-3 py-2.5 text-sm flex items-center gap-2">
      ...
      {accountDeleted && (
        <div className="mb-4 bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 text-gray-600 dark:text-gray-400 rounded-xl px-3 py-2.5 text-sm flex items-center gap-2">
      ...
      {tokenError && (
        <div className="mb-4 bg-rose-50 dark:bg-rose-900/30 border border-rose-200 dark:border-rose-700 text-rose-700 dark:text-rose-400 rounded-xl px-3 py-2.5 text-sm">
          {t('login.status.tokenError')}
        </div>
      )}

      {/* Login/Register tabs */}
      <div className={`flex rounded-xl bg-gray-100/80 dark:bg-gray-800/80 p-1 ${mode === 'register' ? 'mb-3' : 'mb-5'}`}>
        <button type="button" className={`flex-1 rounded-lg py-2 text-sm font-semibold transition-all ${mode === 'login' ? 'bg-white dark:bg-gray-700 shadow-sm text-violet-600 dark:text-violet-400' : 'text-gray-400 hover:text-gray-600 dark:hover:text-gray-300'}`}
          onClick={() => switchMode('login')}>{t('login.tabs.login')}</button>
        <button type="button" className={`flex-1 rounded-lg py-2 text-sm font-semibold transition-all ${mode === 'register' ? 'bg-white dark:bg-gray-700 shadow-sm text-violet-600 dark:text-violet-400' : 'text-gray-400 hover:text-gray-600 dark:hover:text-gray-300'}`}
      ...
      <form onSubmit={handleSubmit} className="space-y-3">
        ...
        <div>
          <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{t('login.fields.email')}</label>
          <input type="email" required value={email} onChange={(e) => setEmail(e.target.value)}
            className={inputClass} placeholder="user@example.com" />
        </div>
        <div>
          <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{t('login.fields.password')}</label>
        ...
        {error && (
          <div className="bg-rose-50 dark:bg-rose-900/30 border border-rose-200 dark:border-rose-700 text-rose-700 dark:text-rose-400 rounded-xl px-3 py-2.5 text-sm">{error}</div>
        )}

        {unverifiedEmail && (
          <div className="bg-amber-50 dark:bg-amber-900/30 border border-amber-200 dark:border-amber-700 text-amber-800 dark:text-amber-400 rounded-xl px-3 py-2.5 text-sm space-y-1.5">
      ...
      {/* Footer */}
      <div className={`${mode === 'register' ? 'mt-4 pt-3' : 'mt-6 pt-4'} border-t border-gray-100 dark:border-gray-800 text-center`}>
