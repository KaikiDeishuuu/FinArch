...
test('renders protected mobile shell with compact top actions', async ({ page }) => {
  ...
  await expect(page.getByLabel(/Exchange|汇率/)).toBeVisible()
  await expect(page.getByLabel(/Settings|设置/)).toBeVisible()
  ...
  await page.getByRole('link', { name: /Budgets|预算/ }).first().click()
